...
public class GameStateEvaluator {
    ...
    private Score getTerminalScore(Game game, Player ai) {
        ...
        if (ai.hasWon()) {
            ...
            return Score.win(Integer.MAX_VALUE - turn * GAME_OVER_TURN_PENALTY);
        }

        // prefer losing later, staying clear of MIN_VALUE so a loss is never mistaken for a failed simulation
        return Score.loss(Integer.MIN_VALUE + 1 + turn * GAME_OVER_TURN_PENALTY);
    }
    ...
    public static class Score {
        ...
        public Score(int value, int availableValue) {
            this(Kind.FINITE, value, availableValue);
        }

        private Score(Kind kind, int value, int availableValue) {
            this.kind = kind;
            this.value = value;
        ...
        public static Score win() {
            return win(Integer.MAX_VALUE);
        }

        public static Score win(int value) {
            return new Score(Kind.WIN, value, value);
        }

        public static Score loss() {
            return loss(Integer.MIN_VALUE);
        }

        public static Score loss(int value) {
            return new Score(Kind.LOSS, value, value);
        }
        ...
        public static boolean meetsThreshold(Score candidate, Score incumbent, int minDelta) {
            ...
            if (candidate.isWin()) {
                return !incumbent.isWin()
                        || (long) candidate.value >= (long) incumbent.value + (long) minDelta;
            }
            if (candidate.isSimFailure() || candidate.isNone()) {
                return false;
            }
            if (candidate.isLoss()) {
                return incumbent.isLoss() && candidate.value > incumbent.value
                        && (long) candidate.value >= (long) incumbent.value + (long) minDelta;
            }
            if (incumbent.isWin()) {
                return false;
        ...
        public Score addDelta(int delta) {
            ...
            return new Score(Kind.FINITE, addFinite(value, delta), addFinite(availableValue, delta));
        }

        private static int addFinite(int value, int delta) {
            long result = (long) value + (long) delta;
            return (int) Math.max(Integer.MIN_VALUE + 1, Math.min(Integer.MAX_VALUE - 1, result));
        }
        ...
        public boolean equals(Score other) {
            if (other == null)
                return false;
            return kind == other.kind && value == other.value && availableValue == other.availableValue;
        }
